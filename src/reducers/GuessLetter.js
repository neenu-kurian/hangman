...
export default (state = initialState, { type, payload } = {}) => {
  switch (type) {
    case GUESS_LETTER:
      const result = showGuess(selectedWord, payload);
      return {
        ...state,
...
function showGuess(word, guess) {
  if (resultArray.join('').toString() !== word && wrongGuess < 10) {
    ...
    if (word.indexOf(guess) > -1) {
      ...
      const result = JSON.parse(JSON.stringify(resultArray).replace(/"\s+|\s+"/g, '"'));
  ...
  return {
    selectedWord, resultArray, wrongGuess, gameStatus,
  };
}
